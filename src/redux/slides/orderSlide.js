...
export const orderSlide = createSlice({
  ...
  reducers: {
    ...
    removeAllOrderProduct: (state, action) => {
      ...
      const itemOrders = state?.orderItems?.filter((item) => !selectedRowKeys.includes(item.product))
      const itemOrdersSelected = state?.orderItems?.filter((item) => !selectedRowKeys.includes(item.product))
      state.orderItems = itemOrders
      state.orderItemsSlected = itemOrdersSelected

    },
    selectedOrder: (state, action) => {
      const {newSelectedRowKeys} = action.payload
      const orderSelected = []
      state.orderItems.forEach((order) => {
        if(newSelectedRowKeys.includes(order.product)){
